    ...
    if ($CurrentPlanet['b_building'] == 0)
    {
        ...
        if ($CurrentQueue != 0)
        {
            ...
            while ($Loop == true)
            {
                ...
                list($Element, $Level, $BuildTime, $BuildEndTime, $BuildMode) = $ListIDArray;
                ...
                else
                {
                    ...
                    {
                        // Ressources insuffisantes pour construire un bâtiment
                        // On recalcule l'heure de fin de construction des bâtiments restants
                        $BuildEndTimeTemp = 0;
                    
                        for ($nb = 0 ; $nb < count($QueueArray) ; $nb++)
                        {
                            list($Element, $BuildLevel, $BuildTime, $BuildEndTime, $BuildMode) = explode(',', $QueueArray[$nb]);
                        
                            if ($nb == 0)
                                $BuildEndTime = time() + $BuildTime;
                            else
                                $BuildEndTime = $BuildEndTimeTemp + $BuildTime;
                            
                            $QueueArray[$nb] = "{$Element},{$BuildLevel},{$BuildTime},{$BuildEndTime},{$BuildMode}";
                            
                            $BuildEndTimeTemp = $BuildEndTime;
                        }
                    }
                }
            } // while
