...
function PlanetResourceUpdate ( $CurrentUser, &$CurrentPlanet, $UpdateTime, $Simul = false ) {
    ...
    if       ($production_level > 100) {
 	...
 	if ( $CurrentPlanet['metal'] <= $MaxMetalStorage ) {
        ...
        $MetalBaseProduc = ($ProductionTime * ($game_config['metal_basic_income'] * $game_config['resource_multiplier']) / 3600);
    ...
    if ( $CurrentPlanet['crystal'] <= $MaxCristalStorage ) {
        $CristalProduction = ($ProductionTime * ($Caps['crystal_perhour'] / 3600) * (0.01 * $production_level));
        $CristalBaseProduc = ($ProductionTime * ($game_config['crystal_basic_income'] * $game_config['resource_multiplier']) / 3600);
    ...
    if ( $CurrentPlanet['deuterium'] <= $MaxDeuteriumStorage ) {
        ...
        $DeuteriumBaseProduc = ($ProductionTime * ($game_config['deuterium_basic_income'] * $game_config['resource_multiplier']) / 3600);
